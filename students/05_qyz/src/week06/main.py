...
def scenario_A_synthetic(results_dir: Path):
    ...
    print(f"✓ 系数验证通过: 估计值 {custom_model.coef_} ≈ 真实值 {true_coef}")

    # 7. 生成报告
    report_path = results_dir / "synthetic_report.md"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# 场景 A: 合成数据测试报告\n\n")
        f.write("## 数据生成过程 (DGP)\n\n")
        f.write(f"- **样本量**: 1000\n")
        f.write(f"- **特征数**: 3\n")
        f.write(f"- **噪声标准差**: 0.5\n")
...
def scenario_B_real_world(results_dir: Path):
    ...
    # 1. 读取数据 - 关键：防止 'NA' 被读作 NaN
    data_path = Path(__file__).parent / "data" / "q3_marketing.csv"

    if not data_path.exists():
        print(f"❌ 错误: 找不到数据文件 {data_path}")
        print("请确保 data/q3_marketing.csv 文件存在")
        return

    # 关键修复：keep_default_na=False 防止 'NA' 被转换为 NaN
    ...
    print(f"\n🌎 北美市场(NA): {len(df_na)} 条数据")
    print(f"🌍 欧洲市场(EU): {len(df_eu)} 条数据")

    if len(df_na) == 0:
        print("❌ 错误: 没有找到北美市场数据")
        print(f"   可用的地区值: {df[region_col].unique()}")
        return
    ...
    if p > 1:
        ...
        print("\n📊 F检验 - 广告投放策略有效性检验")
        print("   原假设 H₀: 所有广告渠道系数 = 0 (广告无效)")

        f_test_na = model_na.f_test(C_overall, d_overall)
        f_test_eu = model_eu.f_test(C_overall, d_overall)

        print(
            f"\n   北美市场(NA): F = {f_test_na['f_stat']:.4f}, p = {f_test_na['p_value']:.6f}"
        )
        print(
            f"   欧洲市场(EU): F = {f_test_eu['f_stat']:.4f}, p = {f_test_eu['p_value']:.6f}"
        )
        ...
        print(
            f"\n   {'✅' if na_effective else '❌'} 北美市场广告策略: {'有效' if na_effective else '无效'}"
        )
        print(
            f"   {'✅' if eu_effective else '❌'} 欧洲市场广告策略: {'有效' if eu_effective else '无效'}"
        )
    else:
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# 场景 B: 真实营销数据分析报告\n\n")

        f.write("## 数据概览\n\n")
        f.write(f"- **总数据量**: {len(df)} 条\n")
        f.write(f"- **北美市场(NA)**: {len(df_na)} 条\n")
        f.write(f"- **欧洲市场(EU)**: {len(df_eu)} 条\n")
        ...
        if p > 1 and f_test_na is not None:
            f.write("\n## F检验结果\n\n")
            f.write("**原假设 H₀**: 所有广告渠道系数 = 0（广告投放无效）\n\n")
            f.write("| 市场 | F统计量 | p值 | α=0.05结论 | 业务解读 |\n")
            f.write("|------|---------|-----|------------|----------|\n")

            na_conclusion = "✅ 拒绝H₀" if na_effective else "❌ 不拒绝H₀"
            eu_conclusion = "✅ 拒绝H₀" if eu_effective else "❌ 不拒绝H₀"
            na_business = "广告投放有效" if na_effective else "广告效果不显著"
            eu_business = "广告投放有效" if eu_effective else "广告效果不显著"

            f.write(
                f"| 北美(NA) | {f_test_na['f_stat']:.4f} | {f_test_na['p_value']:.6f} | {na_conclusion} | {na_business} |\n"
            )
            f.write(
                f"| 欧洲(EU) | {f_test_eu['f_stat']:.4f} | {f_test_eu['p_value']:.6f} | {eu_conclusion} | {eu_business} |\n"
            )
            ...
            if na_effective:
                ...
                f.write("✅ **广告投放策略有效**\n\n")
                f.write(
                    "p值 < 0.05，统计证据表明至少有一个广告渠道对销售额有显著影响。\n\n"
                )
            else:
                f.write("### 北美市场\n")
                f.write("⚠️ **广告投放效果不显著**\n\n")
                f.write("p值 ≥ 0.05，统计证据不足，建议优化广告策略。\n\n")
        ...
        f.write("\n## 可视化\n\n")
        f.write("![市场对比图](market_comparison.png)\n")
...
def main():
    ...
    print("=" * 70)
    print("\n📁 生成的文件:")
    print("   - results/synthetic_report.md    (合成数据测试报告)")
    print("   - results/real_world_report.md   (真实数据分析报告)")
    print("   - results/market_comparison.png  (残差对比图)")
